...
siria = Token:match("(%d+)")
redis:set(siria..":VERSION",1)
redis:set(siria..":SUDO_ID:",SUDO_USER)
redis:set(siria..":DataCenter:",'German')
redis:set(siria..":UserNameBot:",BOT_User)
redis:set(siria..":NameBot:",BOT_NAME)
redis:hset(siria..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3siria_INSTALL","Yes")
info = {} 
...
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @siria_TEAM_3 }• }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE siria source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JACK : @V_P_E  }•\e[0m"

echo -e "\e[38;5;77m      siria ✾ @V_P_Ee[0m"
...
siria = Token:match("(%d+)")
our_id = tonumber(siria)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(siria..":UserNameBot:")
SUDO_ID = tonumber(redis:get(siria..":SUDO_ID:"))
SUDO_USER = redis:hgetall(siria..'username:'..SUDO_ID).username
version = redis:get(siria..":VERSION")
DataCenter = redis:get(siria..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..siria..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(siria..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(siria..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(siria..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 1726705278 then 
msg.TheRank =  'المبرمج سوريا'
msg.Rank = 1

elseif msg.sender_user_id_ == 1465250253 then 
msg.TheRank =  'البابا عرابي  '
msg.Rank = 1

elseif msg.sender_user_id_ == 1761565061 then 
msg.TheRank =  ' روح قلب سوريا. '
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(siria.. :SUDO_BOT: ,msg.sender_user_id_) then 
...
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(siria..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(siria..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(siria..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(siria..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(siria..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(siria..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(siria..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(siria..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(siria..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(siria..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(siria..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قناة سورس سوريا ⇐ @SOURCE_SYRIA
...
if msg.GroupActive and redis:get(siria..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(siria..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(siria..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(siria..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.siria then 
if msg.text and ScriptFile.isiria then
for k, siria in pairs(ScriptFile.siria) do
local SearchText = msg.text:match(siria)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.isiria(msg,{msg.text:match(siria)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",siria.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dsiria then
if ScriptFile.dsiria(msg) == false then
return false
end
...
if Plug.siria then 
if msg.text and Plug.isiria then
for k, siria in pairs(Plug.siria) do
local SearchText = msg.text:match(siria)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(siria..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(siria..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.isiria(msg,{msg.text:match(siria)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",siria.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source siria")
end 

end
else
print("The File Script.lua Not Runing in The Source siria")
...
function tdcli_update_callback(data)
	...
	redis:srem(siria..'users',msg.sender_user_id_)
	end
	...
	EditMsg(data.message_.chat_id_,data.message_.id_,'𝐒')
	EditMsg(data.message_.chat_id_,data.message_.id_,'𝐒𝐎𝐔')
	download_file('https://raw.githubusercontent.com/ahmedsiria/siria/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'𝐒𝐎𝐔𝐑𝐂𝐄')
	download_file('https://raw.githubusercontent.com/ahmedsiria/siria/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐈')
	download_file('https://raw.githubusercontent.com/ahmedsiria/siria/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐈𝐑')
	download_file('https://raw.githubusercontent.com/ahmedsiria/siria/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/ahmedsiria/siria/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/ahmedsiria/siria/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'𝐒𝐎𝐔𝐑𝐂𝐄 𝐒𝐈𝐑𝐈𝐀\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس بلاك بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(siria..":SUDO_ID:")
redis:hset(siria..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(siria..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(siria..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(siria..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(siria..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(siria..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(siria..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(siria..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(siria..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(siria..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(siria..'group:add'..data.chat_.id_) then
	redis:set(siria..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(siria..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(siria..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(siria..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	siria..'group:name'..k,IDS.Title,
	siria..'num_msg_siria'..k,5,
	siria..'group:add'..k,true,
	siria..'lock_link'..k,true,
	siria..'lock_spam'..k,true,
	siria..'lock_webpage'..k,true,
	siria..'lock_markdown'..k,true,
	siria..'lock_flood'..k,true,
	siria..'lock_bots'..k,true,
	siria..'mute_forward'..k,true,
	siria..'mute_contact'..k,true,
	siria..'mute_document'..k,true,
	siria..'mute_inline'..k,true,
	siria..'lock_username'..k,true,
	siria..'replay'..k,true
	)
	redis:sadd(siria..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(siria..'username:'..ID,'username',user)
	redis:sadd(siria..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(siria..'username:'..ID,'username',user)
	redis:sadd(siria..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(siria..'username:'..ID,'username',user)
	redis:sadd(siria..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(siria..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(siria..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(siria..'username:'..data.user_.id_).username
	...
	redis:hset(siria..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
